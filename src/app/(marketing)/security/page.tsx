...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "HermesPass Trust Center — Security, Compliance, Audit Chain",
    description:
...
export default function SecurityPage() {
    ...
    <SiteShell>
      <Section className="grid-backdrop">
        ...
        <h1 className="mt-3 max-w-3xl text-3xl font-semibold tracking-tight sm:text-5xl">
          Security posture, compliance artifacts and a public audit-chain explanation
        </h1>
        <p className="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
          HermesPass exists to make autonomous agents accountable, so we hold our own platform to
          the same standard: bounded authority, attributable actions and evidence that can be
          checked rather than trusted. This page describes how the platform is built and which
          documents we share with enterprise reviewers.
        </p>
      ...
      <Section className="border-t border-border">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-4">
          {CHAIN_STEPS.map((s) => (
            <div key={s.n} className="panel p-6">
              <p className="font-mono text-[11px] tracking-[0.22em] text-cyan-accent">{s.n}</p>
        ...
        <div className="panel mt-6 overflow-x-auto p-6">
          <p className="text-xs font-semibold tracking-wide uppercase">Block structure</p>
  ...
  signature      : signature over the block contents
          ...
          <p className="mt-4 max-w-2xl text-sm text-muted-foreground">
            Verification recomputes each block&apos;s hash and checks that it matches the{" "}
            <span className="font-mono text-xs">previous_hash</span> stored by its successor. A
            single altered field breaks the chain from that point forward, which is why the log is
            described as tamper-evident rather than merely access-controlled.
          </p>
      ...
      <Section className="border-t border-border">
        ...
        <p className="mt-6 max-w-2xl text-sm text-muted-foreground">
          If your review requires an artifact not listed here, tell us what it is and we will
          confirm whether we can provide it, provide an equivalent, or state plainly that it does
          not exist yet.
        </p>
      </Section>
        ...
        <div className="panel mt-8 flex flex-wrap items-center gap-4 p-6">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              ...
              <Link
                href="/contact"
                className="text-emerald-accent underline-offset-4 hover:underline"
              >
                contact form
              </Link>{" "}
              and mark the message as a security report. Please do not test against production
              tenants you do not own.
            </p>
          </div>
        </div>
      </Section>
