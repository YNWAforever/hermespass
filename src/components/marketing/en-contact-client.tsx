...
export function EnContactClient() {
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_1.1fr]">
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              Thirty minutes, technical, no slides required. Bring one agent workflow and we&apos;ll
              walk through passport issuance, the policy decision it would receive, the spend limits
              it needs and the audit evidence it produces.
            </p>

            <ul className="mt-8 space-y-4">
              <li className="flex gap-3 text-sm">
                <Mail className="mt-0.5 size-4 text-cyan-accent" />
                <span className="text-muted-foreground">hello@hermespass.asia</span>
              </li>
              <li className="flex gap-3 text-sm">
                <MapPin className="mt-0.5 size-4 text-cyan-accent" />
                <span className="text-muted-foreground">Hong Kong SAR · Singapore</span>
              </li>
            </ul>
          ...
          {sent ? (
            <div className="panel flex flex-col items-start justify-center p-8">
              ...
              <h2 className="mt-4 text-xl font-semibold">Thanks, {form.name.split(" ")[0]}.</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Your briefing request is in. We&apos;ll follow up at {form.email} within one
                business day.
              </p>
              <Button
                variant="outline"
            ...
            <form onSubmit={submit} className="panel space-y-5 p-6 sm:p-8">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  {errors.region ? <p className="text-xs text-risk-high">{errors.region}</p> : null}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="message">What are your agents doing today?</Label>
                <Textarea
                  id="message"
                  rows={5}
                  value={form.message}
                ...
                />
                {errors.message ? <p className="text-xs text-risk-high">{errors.message}</p> : null}
              </div>
